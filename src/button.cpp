...
BTN_BMP::BTN_BMP( sf::Texture* txt, int nx, int ny, int nw, int nh, std::function <void(GameController*)> no ) : BTN( nx, ny, nw, nh, no ),
    rect( sf::Vector2f( nw, nh ) ){
    rect.setPosition( x, y );
    rect.setOutlineThickness( 1 );
    rect.setOutlineColor( sf::Color( 1, 1, 1 ) );
    sprt.setTexture( *txt );
    sprt.setPosition( x + 4, y + 4 );
}
