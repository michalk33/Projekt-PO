...
void enemy_done( GameController* gc ){
    if( gc->es.opponent_placed &&
        gc->es.opptr->positions[gc->es.opptr->positions.size()-1].first == gc->es.opptr->positions[0].first &&
        gc->es.opptr->positions[gc->es.opptr->positions.size()-1].second == gc->es.opptr->positions[0].second ){
            gc->es.opptr->positions.pop_back();
            gc->brd->opponents.push_back( gc->es.opptr );
            gc->es.opponent_placed = false;
            gc->es.opptr = nullptr;
    }
}
