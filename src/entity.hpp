...
protected:
    ...
    bool clockwise;
public:
    Opponent( enum weight_types wght, enum speed_types spd, int x, int y );
    ~Opponent();
    void add_position( int nx, int ny );
    void run( Board* brd, bool focus ) override;
    void display( sf::RenderWindow* dest, int bx, int by, int tile_size, int brx, int bry, int w, int h, graphics* GRPH ) override;
    enum entity_types type() override;
    enum weight_types weight_type() override;
    ...
    friend void enemy_done( GameController* );
};
...
public:
    ...
    friend class Entity;
    friend class GameController;
    void display_focus( sf::RenderWindow* dest, int bx, int by, int tile_size, int brx, int bry, int w, int h, graphics* GRPH );
};
